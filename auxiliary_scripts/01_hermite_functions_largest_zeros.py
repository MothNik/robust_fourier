...
represent the largest zeros of the Hermite functions with a decent accuracy close to
machine precision.
...
# whether to overwrite the diagnostic plot
OVERWRITE_DIAGNOSTIC_PLOT = True
...
relative error is ``{max_abs_rel_error:.2e}``.
...
SPLINE_DEGREE = 5
# the maximum permitted absolute relative error of the root positions
MAX_ABS_REL_ERROR = 1e-12
...
if __name__ == "__main__":
    ...
    try:
        ...
        orders, outerm_root_positions = reference_data[::, 0], reference_data[::, 1]
    ...
    # otherwise, the reference data is computed
        ...
        outerm_root_positions = np.empty_like(orders, dtype=float)

        progress_bar = tqdm(total=len(orders), desc="Computing outermost roots")
        for idx, n in enumerate(orders):
            outerm_root_positions[idx] = roots_hermite(n=n)[0][-1]
            progress_bar.update(1)

        # the reference data is stored
        np.save(
            reference_file_path,
            np.column_stack((orders, outerm_root_positions)),
            allow_pickle=False,
        )
    ...
    # maximum absolute relative error drops below the threshold
    max_abs_rel_error = np.inf
    s_value = 1e-10
    weights = np.reciprocal(outerm_root_positions)  # all > 0
    while max_abs_rel_error > MAX_ABS_REL_ERROR and s_value > 1e-30:
        tck = splrep(
            x=orders,
            y=outerm_root_positions,
            w=weights,
            k=SPLINE_DEGREE,
            s=s_value,
        )
        outerm_root_positions_approx = splev(x=orders, tck=tck)

        max_abs_rel_error = np.max(
            weights * np.abs(outerm_root_positions - outerm_root_positions_approx)
        )
        s_value /= 10.0**0.25
    ...
    ax[0].plot(  # type: ignore
        orders,
        outerm_root_positions,
        color="red",
        label="SciPy Accurate Roots",
    )
    ax[0].plot(  # type: ignore
        orders,
        outerm_root_positions_approx,
        color="#00CCCC",
    ...
    ax[1].plot(  # type: ignore
        orders,
        100.0 * (outerm_root_positions - outerm_root_positions_approx) * weights,
        color="#00CCCC",
        zorder=2,
        label="Difference",
    )
    ax[1].axhline(  # type: ignore
        MAX_ABS_REL_ERROR * 100.0,
        color="#FF007F",
        linestyle="--",
        label="Threshold",
        linewidth=2.0,
    ...
    ax[1].set_ylabel("Relative Error (%)")  # type: ignore
    ...
    # the plot is stored (if enabled)
    if OVERWRITE_DIAGNOSTIC_PLOT:
