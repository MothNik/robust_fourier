...
error is ``{max_abs_error:.2e}`` with respect to ``scipy.special.roots_hermite``.
...
SPLINE_DEGREE = 5
# the maximum permitted absolute error of the root positions
...
if __name__ == "__main__":
    ...
    while max_abs_error > MAX_ABS_ERROR and s_value > 1e-30:
        tck = splrep(
            x=orders,
        ...
        max_abs_error = np.abs(
            outerm_root_positions - outerm_root_positions_approx
        ).max()
        s_value /= 10.0**0.25
