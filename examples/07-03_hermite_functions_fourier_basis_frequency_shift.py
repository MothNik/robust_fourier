...
for index, order in enumerate(ORDERS):
    ...
    # the "Center" label is a bit tricky here because it has to be placed in the
    # real part of the time/space domain for odd, and in the imaginary part for even
    # orders
    complex_axis = "imaginary" if ut.is_even(order) else "real"
    ...
    for iter_i in range(0, ax.size):  # type: ignore
        ...
        x_center = 0.0 if is_time_space_axis else FREQUENCY_CENTER
    ...
    for iter_i in range(0, ax.size):  # type: ignore
        ...
        if is_time_space_axis:
            ...
            desired_complex_axis = "imaginary" if ut.is_even(order) else "real"
        else:
